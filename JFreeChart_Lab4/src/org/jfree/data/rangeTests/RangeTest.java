package org.jfree.data.rangeTests;
...
public class RangeTest {
	...
	public void constructorLowerBoundGreater() {
		...
		assertEquals("The lower bound value should be null", testRange, null);
	}
	...
	public void intersectLowerOnBoundaryTest() {
		Range myRange = new Range(0.0, 2.0);
		lower = 0.0;
		upper = 2.0;
		assertTrue("These ranges should overlap", myRange.intersects(lower,upper));
	}
    ...
    // Mutation: Check lower bound
    @Test
    public void shiftByPositiveAllowZeroCheckLowerBound() {
		testRange = new Range(2, 6);
		Range testShiftedRange = Range.shift(testRange, 158, true);
		assertEquals("The lower bound should be 160", 160, testShiftedRange.getLowerBound(), .000000001d);
	}
	
    // Mutation: Check lower bound for a non-crossing positive shift where range is (0.0, 0.0)
    @Test
    public void shiftByPositiveWithZeroRangeCheckLowerTest() {
    	testRange = new Range(0.0, 0.0);
    	Range resultRange = Range.shift(testRange, 0.5, false);
    	assertEquals("The lower bound should be 0.5", 0.5, resultRange.getLowerBound(), .000000001d);
    }
    
    // Mutation: Check upper bound for a non-crossing positive shift where range is (0.0, 0.0)
    @Test
    public void shiftByPositiveWithZeroRangeCheckUpperTest() {
    	testRange = new Range(0.0, 0.0);
    	Range resultRange = Range.shift(testRange, 0.5, false);
    	assertEquals("The lower bound should be 0.5", 0.5, resultRange.getUpperBound(), .000000001d);
    } 
	...
	@Test
	// This tests a value that will fail if it is negated
	public void negateConstrainedValueTest() {
		expectedValue = 5;
		constrainValue = -6;
		Range myRange = new Range(5, 7);
		double actualValue = myRange.constrain(constrainValue);
		assertEquals("Actual value should be 5", expectedValue, actualValue, .000000001d);
	}
	
	@Test
	// Tests a value 0.5 below the range
	public void justBelowRangeConstrainTest() {
		expectedValue = 5;
		constrainValue = 4.5;
		Range myRange = new Range(5, 7);
		double actualValue = myRange.constrain(constrainValue);
	...
	@Test
	// Tests a value 0.5 below the range
	public void justAboveRangeConstrainTest() {
		expectedValue = 7;
		constrainValue = 7.5;
		Range myRange = new Range(5, 7);
		double actualValue = myRange.constrain(constrainValue);
		assertEquals("Actual value should be 7", expectedValue, actualValue, .000000001d);
	}
    ...
    // Test if range contains a number on the lower boundary
    @Test
    public void containsLowerBoundTest() {
    	exampleRange = new Range (5, 20);
    	assertTrue("The range of 5 and 20 should contain 5", exampleRange.contains(5));
    }
    
    // Test if range contains a number on the upper boundary
    @Test
    public void containsUpperBoundTest() {
    	exampleRange = new Range (5, 20);
    	assertTrue("The range of 5 and 20 should contain 20", exampleRange.contains(20));
    }
    
    // Test with negative lower bound for range
    @Test
    public void containsWithNegativeLowerBoundTest() {
    	exampleRange = new Range (-5, 20);
		assertTrue("The range of -5 and 20 should contain -4", exampleRange.contains(-4));
    }
    
    // Test with value one below lower bound
    @Test
    public void containsBelowLowerBoundTest() {
    	exampleRange = new Range (1, 5);
    	assertFalse("Range should not contain 0", exampleRange.contains(0));
    }
    
    // Test with value one above upper bound
    @Test
    public void containsAboveUpperBoundTest() {
    	exampleRange = new Range (1, 5);
    	assertFalse("Range should not contain 6", exampleRange.contains(6));
    }    
	...
	public void hashcodeForSame() {
		...
		assertEquals("Hashcode for two similar ranges are the same.", range2.hashCode(), range1.hashCode());
	}
	...
	public void hashcodeForDifferent() {
		...
		assertFalse("Hashcode for two different ranges are not the same.", range1.hashCode() == range2.hashCode());
	}
	
	// Mutation: Testing that the right hash code is printed for all positive range
	@Test
	public void hashCodeForRangeIsCorrectAllPositive() {
		Range myTestRange = new Range(0, 1);
		int hash = 1072693248;
		assertEquals("Hash code should be 1072693248", hash, myTestRange.hashCode());
	}
	
	// Mutation: Testing that the right hash code is printed for all negative range
	@Test
	public void hashCodeForRangeIsCorrectAllNegative() {
		Range myTestRange = new Range(-2.0, -1.0);
		int hash = 2146435072;
		assertEquals("Hash code should be 1072693248", hash, myTestRange.hashCode());
	}
	
	// Mutation: Testing that the right hash code is printed for a neg-to-pos range
	@Test
	public void hashCodeForRangeIsCorrectStraddle() {
		Range myTestRange = new Range(-1.0, 1.0);
		int hash = -31457280;
		assertEquals("Hash code should be -31457280", hash, myTestRange.hashCode());
	}
	...
	// Mutation: test a factor of zero, check upper bound
	@Test
	public void scaleZeroFactorUpperBoundTest() {
		Range myRange = new Range(1.0, 2.0);
		Range resultRange = Range.scale(myRange, 0);
		assertEquals("The upper bound should be 0", 0, resultRange.getUpperBound(), .000000001d);
	}
	
	// Mutation: test a factor of zero, check lower bound
	@Test
	public void scaleZeroFactorLowerBoundTest() {
		Range myRange = new Range(1.0, 2.0);
		Range resultRange = Range.scale(myRange, 0);
		assertEquals("The lower bound should be 0", 0, resultRange.getLowerBound(), .000000001d);
	}	
	...
	// Mutation: check that upper bound is correct
	@Test
	public void expandWithDifferentUpperMarginTest() {
		Range originalRange = new Range(1, 2);
		double lower = 0.5;
		double upper = 1.0;
		Range resultRange = Range.expand(originalRange, lower, upper);
		assertEquals("The upper bound of the range should be 3", 3, resultRange.getUpperBound(), .000000001d);
	}
	
	// Mutation: check that upper bound is correct
	@Test
	public void expandWithDifferentLowerMarginTest() {
		Range originalRange = new Range(1, 2);
		double lower = 0.5;
		double upper = 1.0;
		Range resultRange = Range.expand(originalRange, lower, upper);
		assertEquals("The lower bound of the range should be 0.5", 0.5, resultRange.getLowerBound(), .000000001d);
	}	
	...
	// Mutation: Check that null range upper boundary is correct
	@Test
	public void expandToIncludeNullUpperBoundTest() {
		Range nullRange = null;
		Range resultRange = Range.expandToInclude(nullRange, 1);
		assertEquals("The upper bound value should be 1", 1, resultRange.getUpperBound(), .000000001d);
	}

	// Mutation: Check that lower bound for lower expanded range is correct
	@Test
	public void expandToInlcudeOutsideLowerCheckLower() {
		Range resultRange = Range.expandToInclude(new Range(2, 6), 1);
		assertEquals("The lower value will be 1", 1, resultRange.getLowerBound(), .000000001d);
	}
	
	// Mutation: Check that lower bound for upper expanded range is correct
	@Test
	public void expandToInlcudeOutsideUpperCheckLower() {
		Range resultRange = Range.expandToInclude(new Range(2, 6), 7);
		assertEquals("The lower value will be 2", 2, resultRange.getLowerBound(), .000000001d);
	}
